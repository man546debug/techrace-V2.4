...
class DashboardView(context: Context) : View(context) {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val desired = (w * 1.50f).roundToInt()
        val hSpec = MeasureSpec.getSize(heightMeasureSpec)
        val h = if (MeasureSpec.getMode(heightMeasureSpec) == MeasureSpec.EXACTLY) hSpec else desired
    ...
    private fun drawConnectionBar(c: Canvas) {
        ...
        text(c, "TechRace V2.4", 680f, 176f, 19f, muted, Paint.Align.CENTER)
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        val lx = event.x * 1024f / width
        val ly = event.y * 1536f / height
